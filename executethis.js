...
(function (window) {
    ...
    exports.execute = window.execute = execute = function execute(incomingparams, callback) {
        ...
        proxyprinttodiv("execute - inboundparms",incomingparams,99);
        //proxyprinttodiv("execute - callback ",callback.name,99);
        proxyprinttodiv("execute - callback fn ",String(callback),99);
        //*** add if 'test2'
       ...
       //*** add if 'test3'
        ...
        if (params["executethis"]==="test3") {
            ...
            if ((params['executethis'] !== undefined) && (params['executethis'] !== "")
                ...
                proxyprinttodiv('calling fn ', windowFunc.name,99);
                proxyprinttodiv('calling params ', params,99);
                if (windowFunc.length === 1) {
                    resultObj=windowFunc(params);
                    proxyprinttodiv('calling result ', resultObj,99);
                    callback(resultObj);
                } else {
                    proxyprinttodiv('calling callback ',String(callback),99);
                    windowFunc(params, callback);
                }
            } else {
                proxyprinttodiv('calling params II', params,99);
                callback(params);
            }
        }
    };
    ...
    exports.doThis = doThis = function doThis(params, target, callback) {
        ...
        proxyprinttodiv("dothis - inboundparms",params,99);
        proxyprinttodiv("dothis - target ",target,99);
        proxyprinttodiv("dothis - callback ",String(callback),99);
        //*** add if 'test4'
                ...
                if ((whatToDoList !== undefined) && (whatToDoList != "")) { // make sure we have a list from config, if not just go execute it
                    for (w in whatToDoList) {
                        ...
                        if (howToDo instanceof Function) {
                            ...
                            proxyprinttodiv("dothis II - inboundparms",params,99);
                            proxyprinttodiv("dothis II - callback ",String(callback),99);
                    ...
                    if (howToDo instanceof Function && params[target]) {
                        ...
                        if (howToDo instanceof Function) {
                            // howToDo(params, function(results) { callback(results); }); *** changed by roger
                            proxyprinttodiv("dothis III - parms",params,99);
                            proxyprinttodiv("dothis III - callback ",String(callback),99);
                            howToDo(params, callback);
                        }
    ...
    exports.executethis = window.executethis = executethis = function executethis(inboundparms, targetfunction) {
...

        {
            ...
            proxyprinttodiv('Function executethis fn', targetfunction.name,99);
            proxyprinttodiv('Function executethis length', targetfunction.length,99);
