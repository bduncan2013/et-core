(function(window) {
    ...
    // var isSynchronous = configuration.MongoAddEditPrepare[0].synchronous;

    //     if (!isSynchronous) {
    //         MongoAddEditPrepare([], widset, "", attr,function(err, ret){
    //             AddedObject = ret;
    //             callback(AddedObject);
    //         });
    //     } else {
    ...
    config.configuration.MongoAddEditPrepare = {};
    config.configuration.MongoAddEditPrepare.synchronous = true;

    config.configuration.AddMongoRelationship = {};
    config.configuration.AddMongoRelationship.synchronous = true;

    config.configuration.addcleanparameters = {};
    config.configuration.addcleanparameters.synchronous = true;

    config.configuration.addwidmaster = {};
    config.configuration.addwidmaster.synchronous = true;

    config.configuration.AddWidParameters = {};
    config.configuration.AddWidParameters.synchronous = true;

    config.configuration.AddMaster = {};
    config.configuration.AddMaster.synchronous = true;
    ...
    function AddMaster(dtoList, parameterList, widName, dtotype) {
        ...
        for (childrentype in ChildrenListobj) { // step through all direct children
            ...
            if (Object.keys(parameterindexobj).length !== 0) {
                for (var currentchild in parameterindexobj) {
                    ...
                    if (countKeys(ParametersToAdd) !== 0) {
                        ...
                        proxyprinttodiv('Function AddMaster : ChildrendtoList - 222 wid+childdto A-', {
                            "widtoadd": widtoadd,
                            "ChildrendtoList": ChildrendtoList
                        });
                        proxyprinttodiv('Function AddMaster : ParametersToAdd - 222, childdto+childparameters A- ', {
                        ...
                        var x = AddMongoRelationship(ParentWid, ChildWid, "attributes");
            ...
            if (countKeys(ParametersToAdd) !== 0) {
                ...
                proxyprinttodiv('Function AddMaster : ChildrendtoList - 444 wid+childdto B- ', {
                    "widtoadd": widtoadd,
                    "ChildrendtoList": ChildrendtoList
                });
                proxyprinttodiv('Function AddMaster : ParametersToAdd - 444, childdto+childparameters B-', {
                    "childrentype": childrentype,
                    "ParametersToAdd": ParametersToAdd
                });
                ChildWid = AddMaster(ChildrendtoList, ParametersToAdd, widtoadd, childrentype);
