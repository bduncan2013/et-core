...
//                        throw ({'my_error': 'potatoes'});
    ...
    exports.addwid = addwid = function addwid(object, dtoobject, command, callback) {
        proxyprinttodiv("addwid object", object, 99);
        proxyprinttodiv("addwid dtoobject", dtoobject, 99);
        proxyprinttodiv("addwid command", command, 99);
        var currentobject={}
        ...
        async.series([
            function step1(step1_callback) { // getwidmaster
                // proxyprinttodiv("addwid step1 getwidmaster result", res, 99);
                // if (object['wid']) {
                // execute({
                //     "executethis": "getwidmaster",
                //     "wid": object['wid'],
            ...
            function step2(step2_callback) { // deepfilter step...should create a guid for an empty wid
                ...
                // add wid = guid if its missing
                if (!object.hasOwnProperty("wid")) {
                    object["wid"] =  "undefined";
                }

                proxyprinttodiv("addwid step2 before to deepfilter object", object, 99);
            ...
            function step4(step4_callback) {
                ...
                execute(object, function (err, res) {
                    ...
                    if (err && Object.keys(err).length > 0) {
                        ...
                        // 
                        proxyprinttodiv("this was added", res, 99);
                        step4_callback(err, res);
                    }
                });
                //step4_callback(err, res)
