    ...
    //try {
        ...
        if (widName) {
            ...
            if ((datastore==='localstorage') || (datastore==='localstore')) {
                ...
                //addtoangularstorage
                callback(null, inputWidgetObject);
            } 
...
(function (window) {
    ...
    exports.tolowerparameters = tolowerparameters = function tolowerparameters(parameters, defaults_object, filter_object, deleteflag) {
        ...
        if (Object.keys(filter_object).length > 0) {filter_object = ConvertToDOTdri(filter_object)}
        ...
        proxyprinttodiv("tolowerparameters output I", output, 88);
                ...
                if (isObject(val)) {
                    ...
                    for (var property in val) {
                        if (!output[eachparam].hasOwnProperty(property)) {
                            //extend(true, output[eachparam], val);
                            output[eachparam][property]={};
                            extend(true, output[eachparam][property], val[property]);
                        }
                    }
                } else {
        ...
        if (Object.keys(filter_object).length > 0) {
            for (var eachparam in filter_object) { // create filtered results
                proxyprinttodiv("tolowerparameters output eachparam", eachparam, 88);
                if (output.hasOwnProperty(eachparam)) {
                    filteredobject[eachparam] = output[eachparam]
                    proxyprinttodiv("tolowerparameters filteredobject[eachparam]", filteredobject[eachparam], 88);
        ...
        if (Object.keys(output).length > 0) {output = ConvertFromDOTdri(output)}
        if (Object.keys(filter_object).length > 0) {filteredobject= ConvertFromDOTdri(filteredobject)}

        proxyprinttodiv("tolowerparameters output IV", output, 88);
        proxyprinttodiv("tolowerparameters filteredobject IV", filteredobject, 88);
        return {
            output: output,
